...
export type EntityCategory = 
  ...
  | 'villain'     // Main antagonists/bosses
  | 'npc'         // Non-combat NPCs (vendors, quest givers, etc.)
  | 'slayer'      // Combat-focused allies/mercenaries
  | 'hunter'      // Tracker/ranger type entities
  | 'guardian'    // Defensive/tank entities
  | 'mage'        // Magic users
  | 'assassin'    // Stealth/high damage entities
...
export interface RankTierConfig {
  ...
  rarityWeight: number; // Lower = more common
}
...
export interface Rank {
  ...
  displayName: string; // e.g., "C Rank Hero", "S Rank Villain"
}
 ...
 */
export const CATEGORY_CONFIG: Record<EntityCategory, { label: string; color: string; icon: string }> = {
 ...
 */
export function createRank(tier: RankTier, category: EntityCategory): Rank {
 ...
 */
export function getRankTierConfig(tier: RankTier): RankTierConfig {
 ...
 */
export function getCategoryConfig(category: EntityCategory) {
