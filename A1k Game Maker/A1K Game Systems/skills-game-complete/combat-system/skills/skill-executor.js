...
class SkillExecutor {
  ...
  spawnSummon(skill, x, y, characterState) {
    ...
    console.log(`🎭 Spawned ${characterId} summon: ${skill.name} at (${x}, ${y})`);
    return summon;
  }
  ...
  executeA1BasicAttack(sourceX, sourceY, targetX, targetY, rageActive) {
    console.log('⚔️ [A1 Basic Attack] Executing A1 sword swing combo...');
    ...
    for (let i = 0; i < hitCount; i++) {
      ...
      console.log(`⚔️ [A1 Basic Attack] Spawning sword slash ${i + 1}/${hitCount} at angle ${angle.toFixed(2)}, color ${color}`);
      this.projectileManager.spawnSwordSlash(
        'A1',
    ...
    console.log(`⚔️ [A1 Basic Attack] Spawned ${hitCount} sword slashes. Total swordSlashes: ${this.projectileManager.swordSlashes.length}`);
