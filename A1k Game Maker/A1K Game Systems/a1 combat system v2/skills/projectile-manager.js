...
class ProjectileManager {
  ...
  updateProjectile(proj, deltaTime, enemies) {
    // Update position (vx/vy are in pixels per frame, convert to per second)
  ...
  /**
   * Spawn sword slash (for A1 and MISSY)
   */
  spawnSwordSlash(characterId, x, y, angle, color, radius, damage, delay = 0) {
    setTimeout(() => {
      // Create X-wave projectile
      ...
      const proj = this.spawnProjectile(skillData, x, y, targetX, targetY, {
        speed: 15,
        size: 1.5,
        type: 'xwave'
      });
  ...
  /**
   * Spawn pistol shot (for MISSY)
   */
  spawnPistolShot(x, y, targetX, targetY, damage, homing, speed, delay = 0) {
    setTimeout(() => {
      const skillData = {
      ...
      proj.color = '#FFD700'; // Gold
      proj.homing = homing;
    }, delay * 1000);
  }

  /**
   * Spawn burst shot (for UNIQUE)
   */
  spawnBurstShot(x, y, targetX, targetY, damage, pierce, speed, spread, delay = 0) {
    setTimeout(() => {
      const skillData = {
      ...
      proj.color = '#00ff88'; // Cyan-green
      proj.pierce = true;
      proj.hitsRemaining = pierce;
    }, delay * 1000);
  }
